...
function initMetrics() {
  ...
  metrics.exchanges = probe.metric({
    name: 'Exchanges',
    value: 'N/A'
  });
...
function refreshMetrics(statsAgent) {
  statsAgent.overview(function (err, res, data) {
    if(err){
      var errMatch = err.message.match(/Status code: (\d+)/);
      if(errMatch){
        return pmx.notify("Couldn't connect to Rabbit MQ - " + err.message);
      }
      else if (err.code === 'ECONNREFUSED'){
        return pmx.notify("Couldn't access RabbitMQ management plugin - make sure RabbitMQ management plugin is installed and accessible at: "+ statsAgent.protocol + "://" + statsAgent.hostname);
      }
      else{
    ...
    // # of Exchanges
    var exchanges = dotty.get(data, 'object_totals.exchanges');
    if (exchanges === undefined) {
      exchanges = 'N/A';
    }
    metrics.exchanges.set(exchanges);
